        ...
        <div id="models" className="grid md:grid-cols-2 grid-cols-1 gap-6 mt-[100px]">
            {houses.map((house) => (
                <div key={house.id} className="box-with-darkShadow bg-gray-100 p-[30px] sm:p-[10px] rounded-[15px]">
                    <h3 className="text-xl p-4 font-bold">{house.title}</h3>
                    ...
                    <p className="p-4">{house.nameOne} <span className="font-bold">{house.meters}</span> {house.nameTwo} <span className="font-bold">{house.size}</span></p>
